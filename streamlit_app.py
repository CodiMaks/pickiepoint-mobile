...
# 259200 (free trial right amount of seconds
time.sleep(0.5)
...
if st.session_state.current_page == 'Paraphrase':
    ...
    with paraphrase_col4:
        erase_pre_paraphrase = st.button("❌ Delete", use_container_width=True, help="Clear the paraphrased text")
...
if st.session_state.current_page == 'Settings':
    ...
    if voice_gender_but:
        if st.session_state['voice_gender'] == "♀️ Female Voice":
            ...
            conn = sqlite3.connect('settings_save.db')
            cursor = conn.cursor()
            cursor.execute("UPDATE settings SET voice_gender = ? WHERE customer_id = ?", ("Male", cookie_manager.get(cookie="user_id")))
            conn.commit()
            conn.close()
            ...
            conn = sqlite3.connect('settings_save.db')
            cursor = conn.cursor()
            cursor.execute("UPDATE settings SET voice_gender = ? WHERE customer_id = ?", ("Female", cookie_manager.get(cookie="user_id")))
            conn.commit()
            conn.close()
            ...
            if login_email_placeholder == "":
                ...
                if cursor.fetchone():
                    ...
                    if login_password_placeholder == cursor.fetchone()[0]:
                        ...
                        cookie_manager.set('user_id', customer_id, expires=expiration_date_for_cookie)
        ...
        if not re.match(email_regex, user_email):
            ...
            try:
                ...
                if cursor.fetchone():
                    ...
                    cookie_manager.set('user_id_for_verification_code', customer_id_for_pass_change, expires=cookie_expiration_date)
